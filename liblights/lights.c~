...
/* ACTIVATION LED */
char const*const LED_ACT_FILE
        = "/sys/class/leds/red/brightness";

/* LED TRIGGER*/
char const*const LED_TRIGGER_FILE
        = "/sys/class/leds/red/trigger";

/* DELAY ON/OFF */
char const*const LED_DELAY_ON_FILE
        = "/sys/class/leds/red/delay_on";

char const*const LED_DELAY_OFF_FILE
        = "/sys/class/leds/red/delay_off";

/* RED LED */
char const*const RED_LED_FILE
        = "/sys/class/leds/red/color_red";

/* GREEN LED */
char const*const GREEN_LED_FILE
        = "/sys/class/leds/red/color_green";

/* BLUE LED */
char const*const BLUE_LED_FILE
        = "/sys/class/leds/red/color_blue";
...
blink_red(int level, int onMS, int offMS)
{
	...
	if (nowStatus == 0) {
        	...
        	write_int(RED_LED_FILE, level);
		write_str(LED_TRIGGER_FILE, "timer");
		while (((access(LED_DELAY_OFF_FILE, F_OK) == -1) || (access(LED_DELAY_OFF_FILE, R_OK|W_OK) == -1)) && i<10) {
			ALOGD("LED_DELAY_OFF_FILE doesn't exist or cannot write!!\n");
			led_wait_delay(5);//sleep 5ms for wait kernel LED class create led delay_off/delay_on node of fs
			i++;
		}
		write_int(LED_DELAY_OFF_FILE, offMS);
		write_int(LED_DELAY_ON_FILE, onMS);
	}
	else {
		write_str(LED_TRIGGER_FILE, "none");
        	write_int(RED_LED_FILE, level);
	}
...
blink_green(int level, int onMS, int offMS)
{
	...
	if (nowStatus == 0) {
        	...
        	write_int(GREEN_LED_FILE, level);
		write_str(LED_TRIGGER_FILE, "timer");
		while (((access(LED_DELAY_OFF_FILE, F_OK) == -1) || (access(LED_DELAY_OFF_FILE, R_OK|W_OK) == -1)) && i<10) {
			ALOGD("LED_DELAY_OFF_FILE doesn't exist or cannot write!!\n");
			led_wait_delay(5);//sleep 5ms for wait kernel LED class create led delay_off/delay_on node of fs
        ...
        struct light_state_t const* state)
{
    ...
    if (alpha) { // alpha <> 0 means turn the LED on
	...
	write_int(LED_ACT_FILE, 1);
	} 
	else { // alpha = 0 means turn the LED off
    	red = green = blue = 0;
	write_int(LED_ACT_FILE, 0);
	}
    return 0;
}
